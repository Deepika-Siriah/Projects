...
setwd("D:/SDSU/Spring 2018/MIS 749/Dataset/RDS")
#Read the CSV
...
set.seed(99)
m.H1B.smote.glm <- train(y=y.H1B.smote.train, x=x.H1B.smote.train, 
                                            method = "glm",
...
m.H1B.smote.pca.glm_final <- readRDS("m.H1B.smote.pca.glm.rdata")
...
library(gam)
set.seed(99)
m.H1B.smote.gam <- train(y=y.H1B.smote.train.d, x=x.H1B.smote.train.d, 
                               method="gamSpline", metric = "ROC",
...
##PLS with PCA
set.seed(99)
m.H1B.smote.pca.pls <- train(y=y.H1B.smote.pcr.train, x=x.H1B.smote.pcr.train,  
                             method = "kernelpls",
                             metric = "ROC", tuneLength = 7,
                             trControl = ctrl)

saveRDS(m.H1B.smote.pca.pls, file = "m.H1B.smote.pca.pls.rdata")
readRDS("m.H1B.smote.pca.pls.rdata")
m.H1B.smote.pca.pls
...
library(ipred)
set.seed(99)
m.H1B.smote.bag <- train(y=y.H1B.smote.train,x=x.H1B.smote.train,
...
##Bagging with PCA
set.seed(99)
m.H1B.smote.pca.bag <- train(y=y.H1B.smote.pcr.train, x=x.H1B.smote.pcr.train,  
                                         method = "treebag",
                                         metric = "ROC", tuneLength = 7,
                                         trControl = ctrl)
...
rValues <- resamples(list(Logit=m.H1B.smote.glm, LDA=m.H1B.smote.lda, Rpart=m.H1B.smote.rpart, 
                          Naivebayes=m.H1B.smote.nb, PLS=m.H1B.smote.pls,
                          Glmnet=m.H1B.smote.glmnet, RandomForest=m.H1B.smote.rf, 
                          Bagging=m.H1B.smote.bag))
...
legend(x=.1, y=.8, legend=c("Logit", "LDA", "DT", "NB", "PLS", "Glmnet", "RF", "Bag"), 
       col=c("black","blue","green","red", "pink", "yellow", "Orange", "darkmagenta"),lty=1)
